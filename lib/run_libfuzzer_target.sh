...
if [[ -z "$BINARY" || -z "$COMPONENT" || -z "$TARGET" || -z "$SANITIZER" ]]; then
  echo "usage: run_libfuzzer_target.sh --binary <path> --component <name> --target <name> --sanitizer <asan|hwasan|msan|tsan|ubsan> [--platform <name>] [--artifact-root <dir>] [--corpus-dir <dir>] [--max-total-time <sec>] [--dry-run] [-- extra libFuzzer args]" >&2
  exit 2
fi
...
DEFAULT_CORPUS="$SESSION_DIR/corpus"
...
CMD=("$BINARY" "$CORPUS_DIR" "-artifact_prefix=${CRASH_DIR}/" "-print_final_stats=1" "-max_total_time=${MAX_TOTAL_TIME}")
