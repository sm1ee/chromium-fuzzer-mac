...
if [[ -n "$AUTONINJA_BIN" && -x "$AUTONINJA_BIN" ]]; then
  nice -n 15 "$AUTONINJA_BIN" -j "$NINJA_JOBS" -C out/libfuzzer-trend \
    dawn_wire_lifecycle_sequence_fuzztest_DawnWireLifecycleSequenceFuzzTest_HandlesWireLifecycleSequence_fuzzer
elif [[ -n "$NINJA_BIN" && -x "$NINJA_BIN" ]]; then
  nice -n 15 "$NINJA_BIN" -j "$NINJA_JOBS" -C out/libfuzzer-trend \
    dawn_wire_lifecycle_sequence_fuzztest_DawnWireLifecycleSequenceFuzzTest_HandlesWireLifecycleSequence_fuzzer
else
